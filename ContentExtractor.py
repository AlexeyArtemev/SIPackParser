...
def getContent(packNum):
    ...
    if packNum > 0 and packNum <= int(packLimit) and crsr.execute("""SELECT src FROM pack WHERE pack_ID = {}""".format(packNum)).fetchall()[0][0] == "VK" and zipfile.is_zipfile('/disk/SIGame/Packs/{}'.format(packName)):
        packContentUUID = str(uuid4())
        pack = zipfile.ZipFile('/disk/SIGame/Packs/{}'.format(packName))
        pack.extract("content.xml", path + 'temp')
        os.rename(path + '/temp/content.xml', path + "temp/{}.xml".format(packContentUUID))
        packDict = json.loads(json.dumps(xmltodict.parse(open(path + "temp/{}.xml".format(packContentUUID)).read())))["package"]
        os.remove(path + 'temp/{}.xml'.format(packContentUUID))
